...
namespace {
    ...
    uri_part copy_part(Iterator first, Iterator last,
        ...
        return net_detail::uri_part(part_first, it);
    }
}// namespace

uri_part net_detail::copy_part(const std::string &uri,
                                   string_view::const_iterator &it) {
    return ::copy_part(std::begin(uri), std::end(uri), it);
}

void net_detail::advance_parts(string_view uri_view, uri_parts &parts,
