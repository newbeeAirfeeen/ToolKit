 ...
 *
 * \code
 * net::uri instance("http://cpp-netlib.org/");
...
namespace std {
    template<>
    struct is_error_code_enum<net::uri_error> : public true_type {};
}// namespace std

namespace std {
    template<>
    struct hash<net::uri> {
        std::size_t operator()(const net::uri &uri_) const {
            std::size_t seed = 0;
            std::for_each(std::begin(uri_), std::end(uri_),
                          [&seed](net::uri::value_type v) {
                              std::hash<net::uri::value_type> hasher;
