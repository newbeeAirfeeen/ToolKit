...
namespace VLC {
    ...
    public:
        ...
        explicit MediaTrack(libvlc_media_track_t *c)
            : m_codec(c->i_codec), m_originalFourcc(c->i_original_fourcc), m_id(c->i_id), m_profile(c->i_profile), m_level(c->i_level), m_bitrate(c->i_bitrate) {
    ...
    public:
        ...
        MediaSlave(libvlc_media_slave_t *m)
            : m_type(static_cast<Type>(m->i_type)), m_priority(m->i_priority), m_uri(m->psz_uri) {
        }
