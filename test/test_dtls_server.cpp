...
public:
    ...
    DTLS_Server(asio::io_context &serv,
...
int main()
{
    try
    {
        ...
        asio::ip::udp::endpoint listenEndpoint(listenAddress, 9000);

        asio::ssl::dtls::context ctx(asio::ssl::dtls::context::dtls_server);

        ctx.set_options(asio::ssl::dtls::context::cookie_exchange);

        ctx.use_certificate_file("default.pem", asio::ssl::context_base::pem);
        ctx.use_private_key_file("default.pem", asio::ssl::context_base::pem);

        DTLS_Server<asio::ip::udp::socket> server(_context, ctx, listenEndpoint);
        server.listen();
