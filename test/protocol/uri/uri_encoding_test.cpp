...
TEST(uri_encoding_test, encode_user_info_iterator) {
  ...
  std::string instance;
  net::uri::encode_user_info(std::begin(unencoded), std::end(unencoded),
...
TEST(uri_encoding_test, encode_host_iterator) {
  const std::string unencoded("!#$&'()*+,/:;=?@[]");
  std::string instance;
  net::uri::encode_host(std::begin(unencoded), std::end(unencoded),
			    std::back_inserter(instance));
...
TEST(uri_encoding_test, encode_port_iterator) {
  const std::string unencoded("!#$&'()*+,/:;=?@[]");
  std::string instance;
  net::uri::encode_port(std::begin(unencoded), std::end(unencoded),
			    std::back_inserter(instance));
...
TEST(uri_encoding_test, encode_path_iterator) {
  const std::string unencoded("!#$&'()*+,/:;=?@[]");
  std::string instance;
  net::uri::encode_path(std::begin(unencoded), std::end(unencoded),
			    std::back_inserter(instance));
...
TEST(uri_encoding_test, encode_query_component_iterator) {
  const std::string unencoded("!#$&'()*+,/:;=?@[]");
  std::string instance;
  net::uri::encode_query_component(
...
TEST(uri_encoding_test, decode_iterator) {
  ...
  std::string instance;
  net::uri::decode(std::begin(encoded), std::end(encoded),
		       std::back_inserter(instance));
  ASSERT_EQ("!#$&'()*+,/:;=?@[]", instance);
}
...
TEST(uri_encoding_test, decode_iterator_error_1) {
  ...
  std::string instance;
  ASSERT_THROW(net::uri::decode(std::begin(encoded), std::end(encoded),
				    std::back_inserter(instance)),
	       net::percent_decoding_error);
}
...
TEST(uri_encoding_test, decode_iterator_not_an_error_1) {
  ...
  std::string instance;
  ASSERT_NO_THROW(net::uri::decode(std::begin(encoded), std::end(encoded),
				       std::back_inserter(instance)));
}
